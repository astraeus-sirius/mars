...
module "public_internet_gateway_aws_route" {
  source = "./aws_route"
  create_vpc = var.create_vpc
  vpc_id = local.vpc_id
  subnets = var.public_subnets
  route_table_id         = module.public_route_table.route_table_ids[0].id
  destination_cidr_block = var.public_gateway_route_destination_cidr
...
resource "aws_nat_gateway" "this" {
  ...
  depends_on = [module.aws_internet_gateway.igw_id]
}
...
resource "aws_route_table_association" "redshift_public" {
  ...
  subnet_id = element(aws_subnet.redshift.*.id, count.index)
  route_table_id = element(
    coalescelist(aws_route_table.redshift.*.id, module.public_route_table.route_table_ids.id),
    var.single_nat_gateway || var.create_redshift_subnet_route_table ? 0 : count.index,
  )
}
...
resource "aws_vpn_gateway_route_propagation" "public" {
  ...
  route_table_id = element(module.public_route_table.route_table_ids, count.index)
